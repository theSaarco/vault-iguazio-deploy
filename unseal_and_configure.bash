...
kubectl -n vault cp admin_user_policy.hcl vault-0:/home/vault/admin_user_policy.hcl
...
kubectl -n default-tenant patch deployments.apps jupyter -p '{"spec":{"template":{"spec":{"containers":[{"name":"jupyter","env":[{"name":"MLRUN_SECRET_STORES__VAULT__URL","value":"http://vault-internal.vault:8200"},{"name":"MLRUN_SECRET_STORES__VAULT__ROLE","value":"user:admin"}]}]}}}}'
